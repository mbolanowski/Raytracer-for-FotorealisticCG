...
color::color_t RTAntialiaser::quad_recurse(const mathgik::i2 & pos, const Scene & scene, int recurse_max) const {
    color::color_t average{};
    ...
    for (int y = 0; y < quad_divisions; ++y) {
        for (int x = 0; x < quad_divisions; ++x) {
            mathgik::f2 position = { (float)pos.a + (float)(x + 1) * step, (float)pos.b + (float)(y + 1) * step };
            average.data += camera->get_color_recurse(camera->getRay(position), scene, recurse_max).data * div;
        }
    }
